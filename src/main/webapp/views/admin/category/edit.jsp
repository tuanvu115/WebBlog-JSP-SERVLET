...
<c:choose>
         <c:when test = "${not empty model.id}">
            ...
            <c:if test= "${model.type == 'edit'}">
                 <c:set var = "Title" value="Chỉnh sửa thể loại"/>
            </c:if>
           
            <c:if test ="${model.type == 'view'}" >
                <c:set var = "Title" value="Xem thông tin thể loại"/>
            </c:if>
         </c:when>
         <c:when test = "${empty model.id}">
            <c:set var = "Title" value="Thêm thể loại"/>
         </c:when>
    </c:choose>
    ...
    function addNew(data) {
        $.ajax({
            ...
            success: function (result) {
            	window.location.href = "${CategoryURL}?type=list&page=1&maxPageItem=6&message=insert_success&alert=success";
            },
            error: function (error) {
            	window.location.href = "${CategoryURL}?type=list&maxPageItem=6&page=1&message=error_system&alert=success";
            }
        });
    }
